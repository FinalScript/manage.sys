...
import { createNewEmployee, updateEmployees } from '../api';
...
export const EmployeeModal = ({ hidden, toggle, setEmployeeData, storeId, isNew, employeeId }: Props) => {
    const [form, setForm] = useState<formType>({ employeeName: '', employeeWage: 0, employeeStatus: '', employeeStartingDate: '' });
    ...
    useEffect(() => {
        setForm((prevState) => {
            return { ...prevState, employeeName: '', employeeWage: 0, employeeStatus: '', employeeStartingDate: '' };
        });
    }, [hidden]);
    ...
    const create = () => {
        ...
        if (form.employeeName !== '' && storeId) {
            setTimeout(() => {
                createNewEmployee(storeId, {
                    name: form.employeeName,
                    wage: form.employeeWage,
                    status: form.employeeStatus,
                    startingDate: form.employeeStartingDate,
                })
                    .then((res) => {
    ...
    const update = () => {
        ...
        if (storeId && employeeId) {
            setTimeout(() => {
                updateEmployees(storeId, employeeId, {
                    name: form.employeeName,
                    wage: form.employeeWage,
                    status: form.employeeStatus,
                    startingDate: form.employeeStartingDate,
            ...
            <div className='relative px-4 flex flex-col justify-center mx-auto w-full max-w-xl h-full'>
                <form
                    ...
                    <div className='flex justify-between items-center p-2 pl-5 right-0'>
                        <h1 className='font-medium text-gray-900 dark:text-white text-xl'>{isNew ? 'Create a new employee' : 'Update employee'}</h1>
                        <button
                            onClick={() => {
                                toggle();
                                if (error !== '') {
